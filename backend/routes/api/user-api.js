...
UserApi.prototype.addInActiveUserData = async function(req, res) {
    ...
    var tenantData = await MongoDB.getAllData(
    ...
    var userData = await MongoDB.getAllData(
        AppConstants.DB_NAME,
        AppConstants.USER_LIST,
        queryObj
    );

    var userCount = tenantData[0].userCountLimit
        ? tenantData[0].userCountLimit
        : AppConstants.DEFAULT_USER_COUNT;
    ...
    if (tenantData[0].users.length >= userCount) {
        ...
        var activateURL =
            userObj.siteURL +
            '/userAuth?action=activateUser&userID=' +
            userInsertObj._id +
            '&activationCode=' +
            activationCode +
            '&tenantID=' +
        ...
        // Send warning alert
        var emailBodyToSend =
            'Administrator has invited you to join the ' +
            userObj.email +
            ' account on xSUM.<br>' +
            ...
            activateURL +
            '</a> <br>' +
            'Your temporary password is: <b>' +
            userPasswordBeforEncript +
            '<b> <br>';

        var queryToGetLoggedUser = { email: userObj.loggedUserEmail };
        var loggedUserData = await MongoDB.getAllData(
            AppConstants.DB_NAME,
            AppConstants.USER_LIST,
            queryToGetLoggedUser
        );
        ...
        res.send({
            message: AppConstants.RESPONSE_SUCCESS,
            user: { email: userInsertObj.email }
        });
    }
};
...
UserApi.prototype.updateUserData = async function(req, res) {
    ...
    if (userData.length > 0) {
        ...
        var existingUserData = await MongoDB.getAllData(
            AppConstants.DB_NAME,
            AppConstants.USER_LIST,
            queryObj
        );
...
UserApi.prototype.getUserData = async function(req, res) {
    ...
    userData[0].permissions = {
        canCreate: accessControl
            .can(userData[0].tenants[0].role)
            .createAny(AppConstants.ANY_RESOURCE).granted,
        canRead: accessControl
            .can(userData[0].tenants[0].role)
            .readAny(AppConstants.ANY_RESOURCE).granted,
        canUpdate: accessControl
            .can(userData[0].tenants[0].role)
            .updateAny(AppConstants.ANY_RESOURCE).granted,
        canDelete: accessControl
            .can(userData[0].tenants[0].role)
            .deleteAny(AppConstants.ANY_RESOURCE).granted
    };

    // Get super user permissions
    userData[0].isSuperUser =
        userData[0].tenants[0].role === AppConstants.SUPER_USER;
    ...
    if (userData.length > 0) {
        ...
        if (userData[0].isActive) {
            if (bcrypt.compareSync(userObj.password, storedPassword)) {
                ...
                res.send({
                    message: AppConstants.EMAIL_AND_PASSWORD_NOT_MATCH
                });
            }
        } else {
...
UserApi.prototype.getUserList = async function(req, res) {
    var userObj = req.body;
    var queryObj = {};
    var userList = await MongoDB.getAllData(
        userObj.tenantID,
        AppConstants.USER_LIST,
        queryObj
    );
...
UserApi.prototype.activateUser = async function(req, res) {
    ...
    if (checkForHexRegExp.test(paramObj.userID)) {
        ...
        var userDataToActivate = await MongoDB.getAllData(
            AppConstants.DB_NAME,
            AppConstants.USER_LIST,
            queryObj
        );
        ...
        if (userDataToActivate.length > 0) {
            if (
                userDataToActivate[0].activationCode === paramObj.activationCode
            ) {
                // Activate user
                var userUpdateObj = {
                    isActive: true,
                ...
                if (
                    userDataToActivate[0].activationCode === '' &&
                    userDataToActivate[0].isActive
                ) {
...
UserApi.prototype.updateProfile = async function(req, res) {
    ...
    if (userObj.password)
        updateObj.password = await hashPassword(userObj.password);
